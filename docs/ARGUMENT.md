# ZeroClaw on Solana: the case in full, claim by claim

*The one-page version is [`docs/ONE-PAGER.md`](ONE-PAGER.md). This is the long form: the same
claims, with the reasoning, the controls, and the things that are still open.*
...
**Every link to the rest of the submission lives on one page:**
[`docs/ONE-PAGER.md`](ONE-PAGER.md). It is kept in a single place on purpose, because two copies of
a link list are two things to keep true and only one of them gets checked.
