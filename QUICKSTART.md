...
spoofed feed account, a versioned (v0) message, which this path does not emit and which
would misalign every offset if read as legacy, and two POSITIONAL attacks: our real feed
present in the instruction but seated third behind an attacker's feed and device, and the
same seated second. Those last two matter because Anchor binds accounts by POSITION and
tolerates extras as `remaining_accounts`, so asking only whether our feed appears somewhere
in the list certifies a publish that lands on someone else's.
...
| `curl -s https://x402.perfpilot.dev/price` | under 2s (on Windows see the note below this table) | The earning node answering live. Returns HTTP 402 with ONE price tier (`one feed reading`), the devnet USDC mint, the pay-to address, and a single-use memo nonce that changes on every request. Re-derive the tier count rather than trusting it, because it moved once when the day-pass tier was withdrawn from the deployed binary: `python -c "import urllib.request as u,json; o=u.build_opener(type('N',(u.HTTPErrorProcessor,),{'http_response':lambda s,q,r:r,'https_response':lambda s,q,r:r})); print(len(json.loads(o.open(u.Request('https://x402.perfpilot.dev/price',headers={'User-Agent':'Mozilla/5.0'}),timeout=25).read())['accepts']))"` prints `1`. Note 402 is the correct answer here, so a bare `urlopen` raises and discards the body; the opener above is what reads it on one line. This one is a live demonstration rather than evidence: if the node is down you get a gateway error, whereas the two offline checks above verify from committed bytes. Its SHAPE is checkable though, by a grader that is not ours: `cd scripts/x402-validator && npm ci --silent && node validate-challenge.mjs` runs it past `@x402/core`'s own `PaymentRequiredV2Schema`, with the pre-cutover body as a control that must be rejected. |
